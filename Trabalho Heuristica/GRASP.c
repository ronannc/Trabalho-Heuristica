...
void LocalSearch(ponto *solucao_parcial, node *lista_ponto) {
    ...
    int demanda = max_demanda - GRASP_demanda_coberta_xy(lista_ponto, x, y);
    int aux_demanda = 0;

    /*procurar o melhor ponto a se colocar a facilidade com a solução parcial*/
    for (int i = 0; i < lista_ponto->tam; i++) {
        aux_demanda = GRASP_demanda_coberta_xy(lista_ponto, numPos(lista_ponto, i).cord_x, numPos(lista_ponto, i).cord_y) + demanda;
